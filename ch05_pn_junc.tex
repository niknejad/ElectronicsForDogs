...
Given the above approximations, we now have an expression for the \textbf{charge density}\index{PN-junctions!charge density} in each region of the semi-conductor:
    \begin{equation}
        \rho _0(x) \cong
            \begin{cases} 
...
and:
    ...
    \end{equation}
Where the total potential difference between the $N$-type and $P$-type side is \textbf{built-in potential}\index{PN-junctions!built-in potential}:
    \begin{equation}
...
Unfortunately this cannot be the case.  When we attach a wire to the $PN$-junction, we actually are replaying the same physics that we just described.  For example, a metal is kind of like an $N$-type region, but with very heavy doping.  So it will form a built-in potential, called the \textbf{contact potential}\index{PN-junctions!contact potential}, between the $P$-type and $N$-type regions.  Let's call the contact potential with the $N$-type region as $\varphi_{mn}$ and the contact potential with the $P$-type region as $\varphi_{mp}$.  
