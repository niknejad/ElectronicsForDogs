...
We know from the previous chapter that op-amps are designed to have a single dominant pole.  The transfer function of the op-amp is therefore approximately given by:
    ...
    \end{equation}
In \emph{Eq.~\ref{eq:op_amp_transfer}}, the DC gain is $A_0$, and the $-3\,dB$ frequency is $\omega_{-3dB}$.  In the previous chapter we also derived the \textit{unity-gain bandwidth}\index{Op-amp!Gain-bandwidth product}, $\omega_u = A_0\,\omega_{-3dB}$, which is also known as the  gain-bandwidth product ($GBW$).
...
The \textbf{output voltage swing}\index{Amplifier!output voltage swing} is limited by the voltage supplies, usually omitted from the schematic, but shown explicitly in \emph{Fig.~\ref{fig:opamp_invert_gain_supplies}}.  If the \textit{input voltage times the gain of the amplifier is larger than the supplies}, the output will clip, as shown in \emph{Fig.~\ref{fig:sin_clip}}.  Here we are optimistically assuming that the op-amp can swing all the way to the rails.  In reality, the output swing may be lower than the rails due to the various implementation details.  Most op-amps also have a maximum output current limit, activated to prevent the op-amp from damage in the case of a short circuit or connection of a very small load resistor.  In either case, the output waveform will appear \textbf{"clipped"}\index{Amplifier!clipping}, leading to \textbf{distortion}\index{Amplifier!distortion}.  In most linear amplifier applications, clipping is unwanted and avoided by careful design.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                 FIGURE                   %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
...
The \textbf{"slew rate"}\index{Amplifier!slew rate}, or the maximum possible change at the output, is defined by:
    \begin{align}
...
To understand $SR$-limiting, let's consider a simple unity-gain op-amp amplifier shown in \emph{Fig.~\ref{fig:opamp_unitygain_step_fig}}.  If a step voltage is supplied at the input, the output would ideally follow the exponential settling predicted by simple linear theory:
    \begin{equation}
...
Since $|\cos(\omega t)| \le 1$, the \textit{maximum slope} is given by:
    ...
    \end{equation}
The \textbf{full-power bandwidth}\index{Amplifier!full-power bandwidth} is known as the frequency at which the $SR$-limited distortion starts to occur for an output sinusoid with maximum rated output voltage, $V_{o,max}$:
    \begin{align}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
We already encountered this when we derived the large signal transfer function of the MOS differential pair (see \emph{Fig.~\ref{fig:Diff_amp_large.pdf}}). Also, all of the transistors we have studied have a non-linear $I$-$V$ and $C$-$V$ curve.  This leads to harmonic and intermodulation distortion\index{Amplifier!distortion}.  This is a topic we will cover in detail in other courses, such as a communication circuits or advanced analog circuits course.

As a simple example, in \emph{Fig.~\ref{fig:vdisto}}, the ideal sinusoidal output is distorted due to \textbf{harmonics of the waveform}\index{Waveform harmonics}.  Harmonics are generated due to non-linearity.  For example, if we model a small portion of the $I$-$V$ curve with a Taylor series expansion\index{Taylor expansion}, we have:
    \begin{equation}
