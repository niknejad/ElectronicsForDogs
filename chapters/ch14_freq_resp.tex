...
For any circuit transfer function\index{Transfer function}, the number of poles is a property of the circuit, and does not depend on the choice of input/output terminals.  The number of \textit{poles}\index{Poles} is given by the \textit{number of independent capacitors and inductors}.  The zeros\index{Zeros}, on the other hand, depends on the transfer function, in other words the \textit{choice of the input and output ports}.
...
Let's combine these two equations into a matrix:
    \begin{equation}
        ...
        \end{matrix}\right] 
    \end{equation}
Using Cramer's Rule\index{Cramer's Rule} or another approach, it is easy to show that the transfer function is given by:
    \begin{equation}
...
Applying the \textbf{Miller Theorem}\index{Miller Theorem} to a capacitor, shown in \emph{Fig.~\ref{fig:miller2}}, we have a capacitor that couples two nodes.  In this case, it is across an inverting amplifier, and so the effective capacitance is boosted:
    \begin{equation}
...
Now we apply the Miller Theorem\index{Miller Theorem!Common source analysis} to the $C_{gd}$ feedback capacitor in the MOS amplifier, shown in \emph{Fig.~\ref{fig:cs_amp_cgd}}:  
    \begin{equation}
...
Let's now compute the bandwidth of a common drain amplifier\index{Miller Theorem!Common drain analysis}, also known as the source follower, shown in \emph{Fig.~\ref{fig:cd_amp_ss_cap_load}a}.  We can replace the current source with MOSFET-based current mirror, and we simply model it by $r_{oc}$.  In the small-signal model with capacitors shown in \emph{Fig.~\ref{fig:cd_amp_ss_cap_load}b}, we use the DC small-signal gain and the Miller effect to calculate the input capacitance.  This will allow us to estimate the dominant pole.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                 FIGURE                   %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[H]
