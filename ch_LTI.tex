...
Let's begin with an example calculation.  Suppose we have a low pass filter\index{Filters!Low pass} as shown in Fig.~\ref{fig:lpf}.  If we drive this system with a source $v_s$ and observe the output at $v_o$, we can write the governing equations as follows by using KVL around the loop:
    \begin{equation}
        v_o(t) = v_s(t) - i(t) R
    \end{equation}
    Since
