...
Let's begin with an example calculation.  Suppose we have a \textbf{low pass filter}\index{Filters!Low pass} as shown in \emph{Fig.~\ref{fig:lpf}}.  If we drive this system with a source $v_s$ and observe the output at $v_o$, we can write the governing equations as follows by using KVL around the loop:
    \begin{align*}
        v_o(t) &= v_s(t) - v_r(t)\\
        &= v_s(t) - i(t) R
    \end{align*}
