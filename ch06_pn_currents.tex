...
It is common practice to lump the constants $c_p\,N_T$ into a term called the \textbf{minority carrier lifetime}\index{PN-junction!Minority carrier lifetime} $\tau_p$, because from the above equation it's clear that their product has units of inverse time:
    \begin{equation}
...
In many applications, we are concerned with the diode response to a \textbf{small signal}\index{Diode!Small-signal model}.  In these situations, we usually bias the diode with a DC voltage to the desired \textbf{operating point}\index{Biasing!operating point}, either in forward or reverse biased region.  Then we apply a small incremental signal to the diode.  A good example is when the diode is used as a switch.  In this case we either turn the diode on and rely on its low impedance to pass small signals, or turn it off to isolate small signals from the rest of the circuit. Many mobile phones use a diode exactly as described above to share a single antenna between the receiver and transmitter.
...
For $x = v_d/kT/q$ and $v_d \ll kT/q$ (small-signals less than $26\,mV$),  we can linearize the exponential using a Taylor Series expansion\index{Taylor expansion}:
    ...
    \end{equation*}
In other words, the total current can be written in terms of the \textbf{DC bias}\index{Biasing!DC bias} current and the incremental AC current:
    \begin{equation*}
